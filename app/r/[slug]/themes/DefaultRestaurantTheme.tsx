...
const THEME_MEDIA: Record<string, { images: string[]; video?: string }> = {
  ...
  "pizza-coastal-local": {
    images: [
      "https://images.unsplash.com/photo-1593560708920-61dd98c46a4e?auto=format&fit=crop&w=1800&q=88",
      "https://images.unsplash.com/photo-1579751626657-72bc17010498?auto=format&fit=crop&w=1400&q=86",
      "https://images.unsplash.com/photo-1559339352-11d035aa65de?auto=format&fit=crop&w=1400&q=86",
      "https://images.unsplash.com/photo-1565299624946-b28f40a0ae38?auto=format&fit=crop&w=1400&q=86"
    ]
  },
  "pizza-slice-shop": {
...
function PizzaCoastalLocal({ data }: { data: SiteData }) {
  const { restaurant, branding, website, ordering } = data;

  const sea = branding?.primary_color || "#1F5A62";
  const coral = branding?.secondary_color || "#D86F3D";
  const sand = "#F3E8CF";
  const navy = "#15343A";

  const hero =
    getHeroImage(data, "pizza-coastal-local") ||
    getHeroImage(data, "pizza-slice-shop");
  const heroVideo = website.hero_video_url || null;
    ...
    <main className="coastal-shell">
      ...
      <style jsx global>{`
        ...
        .coastal-header {
          ...
          top: 0;
          z-index: 60;
          background: rgba(243,232,207,.96);
          backdrop-filter: blur(14px);
          border-bottom: 1px solid rgba(21,52,58,.14);
        }
        ...
        .coastal-hero-overlay {
          position: absolute;
          inset: 0;
          background:
            linear-gradient(180deg, rgba(14,40,45,.08) 0%, rgba(14,40,45,.22) 45%, rgba(14,40,45,.82) 100%);
        }

        .coastal-hero-content {
          position: relative;
          z-index: 2;
          max-width: 1250px;
          width: 100%;
          margin: 0 auto;
          padding: 120px 28px 68px;
          color: #fff;
        }
        ...
        .coastal-card {
          ...
          border-radius: 16px;
          overflow: hidden;
          box-shadow: 0 12px 30px rgba(21,52,58,.06);
        }
        ...
        .coastal-hospitality p {
          color: #c5e0e2;
          font-size: 16px;
          line-height: 1.8;
        }

        .coastal-hospitality-grid {
          display: grid;
          grid-template-columns: repeat(2,1fr);
          gap: 12px;
        }

        .coastal-hospitality-card {
        ...
        .coastal-hospitality-card strong {
          display: block;
          font-family: Georgia, "Times New Roman", serif;
          font-size: 23px;
          margin-bottom: 8px;
        ...
        @media(max-width: 640px) {
          ...
          .coastal-nav.right a:not(.coastal-order) {
            display: none;
          }
      ...
      <header className="coastal-header">
        <div className="coastal-header-inner">
          ...
          <a href={`/r/${restaurant.slug}`} className="coastal-brand">
            {website.logo_url ? (
              <img
                src={website.logo_url}
                alt={restaurant.name}
                style={{ maxWidth: 210, maxHeight: 68, objectFit: "contain" }}
              />
            ) : (
              <>
                {restaurant.name}
                <small>LOCAL PIZZA · GOOD TIMES</small>
              </>
            )}
          </a>

          <nav className="coastal-nav right">
            {restaurant.phone && <a href={`tel:${restaurant.phone}`}>{restaurant.phone}</a>}
            {ordering?.online_ordering_url && (
              <a
                className="coastal-order"
                href={ordering.online_ordering_url}
                target="_blank"
                rel="noreferrer"
              >
      ...
      <section className="coastal-hero">
        {heroVideo ? (
          ...
          <img className="coastal-hero-media" src={hero} alt="" />
        ) : null}

        <div className="coastal-hero-overlay" />

        <div className="coastal-hero-content">
          <div className="coastal-kicker">PIZZA · DRINKS · GOOD COMPANY</div>
          <h1>{website.hero_headline || restaurant.name}</h1>
          <p>
            {website.hero_subheadline ||
              branding?.short_description ||
              "A neighborhood pizza place made for easy dinners, cold drinks, family nights, weekend hangs and the people you came to town with."}
          </p>

          <div className="coastal-actions">
            {ordering?.online_ordering_url && (
              <a
                className="coastal-primary"
                href={ordering.online_ordering_url}
                target="_blank"
                rel="noreferrer"
              >
            ...
            <a className="coastal-secondary" href={`/r/${restaurant.slug}/food-menu`}>
              {website.secondary_cta_label || "VIEW MENU"}
            </a>
          </div>
        </div>
      ...
      <section className="coastal-quick">
        <a href={`/r/${restaurant.slug}/food-menu`}>
          <strong>Pizza</strong>
          <span>See The Menu</span>
        </a>
        <div>
          <strong>Cold Drinks</strong>
          <span>Beer · Wine · More</span>
        </div>
        <a href="#gather">
          <strong>Groups</strong>
          <span>Parties · Teams · Events</span>
        </a>
        <a href="#visit">
          <strong>Visit</strong>
          <span>{restaurant.city || "Find Us"}</span>
        </a>
      </section>

      <section className="coastal-intro" id="story">
        <div className="coastal-kicker" style={{ color: coral }}>
          YOUR LOCAL PLACE
        </div>
        <h2>{website.about_title || "Good Pizza. Easy Nights."}</h2>
        <p>
          {website.about_body ||
            branding?.short_description ||
            "The best local restaurants are part dinner plan, part gathering place. We built this one around great food, relaxed hospitality and making it easy to come back again tomorrow."}
        </p>
      </section>

      <section className="coastal-photo-grid">
        <img
          className="coastal-photo-main"
          src={
            getThemeImage(data, "pizza-coastal-local", 1) ||
            getThemeImage(data, "pizza-slice-shop", 1)
          }
          alt=""
        />

        <div className="coastal-side-stack">
          <img
            className="coastal-photo-side"
            src={
              getThemeImage(data, "pizza-coastal-local", 2) ||
              getThemeImage(data, "pizza-slice-shop", 2)
            }
            alt=""
          />
          <img
            className="coastal-photo-side"
            src={
              getThemeImage(data, "pizza-coastal-local", 3) ||
              getThemeImage(data, "pizza-slice-shop", 3)
            }
            alt=""
          />
        </div>
      ...
      {featured.length > 0 && (
        <section className="coastal-menu">
          <div className="coastal-menu-inner">
            <div className="coastal-menu-head">
              <div>
                <div className="coastal-kicker" style={{ color: coral }}>
                  HOUSE FAVORITES
                </div>
                <h2>Order What The Locals Order.</h2>
              </div>
            ...
            <div className="coastal-featured">
              {featured.map((item, index) => (
                <article className="coastal-card" key={item.id}>
                  <img
                    className="coastal-card-photo"
                    src={
                      getThemeImage(data, "pizza-coastal-local", index % 4) ||
                      getThemeImage(data, "pizza-slice-shop", index % 4)
                    }
                    alt=""
                  />

                  <div className="coastal-card-copy">
                    <div className="coastal-card-title">
                      <span>{item.name}</span>
                      {item.price !== null && (
                        <span className="coastal-card-price">
                          ${Number(item.price).toFixed(2)}
                        </span>
                      )}
                    </div>
                    ...
                    {item.description && (
                      <div className="coastal-card-desc">{item.description}</div>
                    )}
                  </div>
                </article>
              ))}
      ...
      <section className="coastal-hospitality" id="gather">
        <div className="coastal-hospitality-inner">
          <div>
            <div className="coastal-kicker">MAKE A NIGHT OF IT</div>
            <h2>More Than A Takeout Box.</h2>
            <p>
              Built for local restaurants that win through atmosphere as much as food:
              drinks, parties, team dinners, catering and the kind of hospitality that turns visitors into regulars.
            </p>
          </div>

          <div className="coastal-hospitality-grid">
            <div className="coastal-hospitality-card">
              <strong>Parties</strong>
              <span>Birthdays, teams, family gatherings and casual group dinners.</span>
            </div>
            <div className="coastal-hospitality-card">
              <strong>Catering</strong>
              <span>Easy group ordering for offices, events and vacation-house dinners.</span>
            </div>
            <div className="coastal-hospitality-card">
              <strong>Drinks</strong>
              <span>Give beer, wine and the bar atmosphere a real place in the brand.</span>
            </div>
            <div className="coastal-hospitality-card">
              <strong>Local Life</strong>
              <span>Feature events, community nights, seasonal traffic and neighborhood energy.</span>
            </div>
          </div>
        </div>
      </section>

      <section className="coastal-review">
        <blockquote>
          “The kind of pizza place you find on vacation and wish you had back home.”
        </blockquote>
        <cite>LOCAL FAVORITE · GREAT FOOD · EASY VIBE</cite>
      </section>

      <section className="coastal-visit" id="visit">
        <img
          className="coastal-visit-photo"
          src={
            getThemeImage(data, "pizza-coastal-local", 0) ||
            getThemeImage(data, "pizza-slice-shop", 0)
          }
          alt=""
        />

        <div className="coastal-visit-copy">
          <div className="coastal-kicker" style={{ color: coral }}>
            COME HANG OUT
          </div>
          <h2>{restaurant.name}</h2>

          {address && <div className="coastal-detail">{address}</div>}
          {restaurant.phone && (
            <div className="coastal-detail">
              <a href={`tel:${restaurant.phone}`}>{restaurant.phone}</a>
            </div>
          )}
      ...
      <footer className="coastal-footer">
        <span>© {new Date().getFullYear()} {restaurant.name}</span>
        <span>Restaurant OS · Built For Independent Restaurants</span>
      </footer>
    </main>
  );
}
