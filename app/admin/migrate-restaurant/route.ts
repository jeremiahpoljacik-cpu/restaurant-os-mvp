...
export async function POST(request: NextRequest) {
  try {
    ...
    // 1) Find an existing auth user by email.
    const {
      data: usersPage,
      error: usersError,
    ...
    // 2) If owner does not exist, create a login with a temporary password.
    if (!ownerUserId) {
      temporaryPassword = `ROS-${crypto.randomUUID().replace(/-/g, "").slice(0, 14)}!`;
    ...
    // 5) Seed the common Restaurant OS records.
    const [
      brandingResult,
      websiteResult,
      orderingResult,
    ...
    // 6) Store the custom domain as pending. We do not touch DNS here.
    if (customDomain) {
